...
接口返回 `entries` 和 `me`。`entries` 每项仅含 `rank`（名次）、
`display_name`（`用户 #` 加内部用户 ID）、`streak_days`（连续天数），
不返回真实用户名或邮箱；匿名标识中的数字不是名次，页面分列展示两者。
`me` 返回自己的 `streak_days`、`rank` 和 `is_trial`：正式账号只要连续天数
至少为 1，就返回全部合格账号中的真实名次，即使超过 50 名；体验账号或
